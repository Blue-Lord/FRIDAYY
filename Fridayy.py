...
def Weather(timeing):                           #weather fumction
    try:
        ...
        if x["cod"] != "404":
            ...
            print("FRIDAYY : Please check your city in the settings as this is invalid.")
            Audfile = open("cookies.txt", "a")
            querytime = (datetime.datetime.now().ctime())
            Audfile.writelines(querytime + "-(User'S CITY IS INVALID!!!) \n")
            Audfile.close()
    except:
        print("FRIDAYY : I am having a problem in getting live weather please check your network-Connection.")
        Audfile = open("cookies.txt", "a")
        querytime = (datetime.datetime.now().ctime())
...
def stocks(tickers):                                      #search for stocks news
    try:
        ...
        print("FRIDAYY : I am having a problem in getting Stock Prices please check your network-Connection")
        Audfile = open("cookies.txt", "a")
        querytime = (datetime.datetime.now().ctime())
        ...
        while True:     
            ...
            if event == 'GET NEWS':                           #get-news button of news search dialogue box
                try:
                    ...
                    print("FRIDAYY : Something went wrong!!!" +
...
def send_an_email(from_address, to_address, subject, message_text, password):            #send email
    try:
        fridayy_mail = '\n \n \n \n                                                        ---This message was sent to you by FRIDAYY  :-)'
        full_message = "{0} {1}".format(message_text, fridayy_mail)
...
def openapps(term):
    if "NOTEPAD" in term:
        ...
        print("can not help  you mother lover")  
...
window = sg.Window('FRIDAYY GUI', layout, location=(0,0) ,icon=r'icon/Fridayy.ico', font=(                 
    'Helvetica', ' 13'), default_button_element_size=(8, 2)).Finalize()
window.maximize()

print("FRIDAYY : Welcome sir")
Audfile = open("cookies.txt", "a")
querytime = (datetime.datetime.now().ctime())
Audfile.writelines(querytime + "-(User ACTIVATED FRIDAYY AND INITIALIZED RELATED PROCESSES!!!) \n")
Audfile.close()

hour = int(datetime.datetime.now().hour)              #-----general function, always work
if hour >= 0 and hour < 12:
    timeing = "FRIDAYY : Good morning, here is the current weather in "
    Weather(timeing)
    Breifing('Morning Briefing', 'Morning News Headlines')     #only morning news will pop up
elif hour >= 12 and hour < 18:
    timeing = "FRIDAYY : Good afternoon, here is the current weather in "
    Weather(timeing)
else:
    timeing = "FRIDAYY : Good evening, here is the current weather in "
    Weather(timeing)
print("\n")
print("Functionalities Currently available : ")
print("Salutations, Date/Time, Current news, Stocks update, Send email, Digital clock, Web browsing, Open apps, IP adress, Games, Exit")
...
if __name__ == '__main__':                              #----------DRIVER CODE STARTS
    while True:     
        ...
        if event in (sg.WIN_CLOSED, 'Exit'):   # quit if exit button or X                #exit at top right
            Audfile = open("cookies.txt", "a")
            querytime = (datetime.datetime.now().ctime())
            Audfile.writelines(querytime + "-(User TERMINATED FRIDAYY AND ALL IT'S RELATED PROCESSES!!!) \n")
            Audfile.close()
            print("\nFRIDAYY: Goodbye sir hope you have a nice day :-)")
            time.sleep(4)
            break
            ...
            with open("Credentials.json") as f:
                ...
                #-------------------- THE END OF THE RENDER-WORD ENGINE . from here, network search starts--------------------
                        ...
                        if "DATE" in query or "TIME" in query:
                            ...
                            print("FRIDAYY : The Date and Time is ",x," respectively sir.")
                            continue  
                            ...
                            try:
                                ...
                                print(
                                    "FRIDAYY : Something went wrong please check if you have a good network Connection.")
                                Audfile = open("cookies.txt", "a")
                                querytime = (datetime.datetime.now().ctime())
                                Audfile.writelines(
                                    querytime + "-(Connection failed WITH NEWSAPI.ORG!!!) \n")
                            ...
                            Audfile.writelines(querytime + "-(User TERMINATED FRIDAYY AND ALL IT'S RELATED PROCESSES!!!) \n")
                            Audfile.close()
                            print("FRIDAYY : Goodbye sir hope you have a nice day :-)")
                            time.sleep(4)
                            break 
                            ...
                            print(
                                "FRIDAYY : I did not understand you, as you can see i am still evolving :-)")
                            Audfile = open("cookies.txt", "a")  
